...
export class ProgressService {
  ...
  async handleIncomingTracking(dto: UpdateProgressDto) {
    ...
    if (dto.currentTime === 0 && dto.eventType === 'LEAVE_PAGE') {
      ...
      if (maxTimeWeHave > 0) {
        this.logger.warn(`🚫 [Backend Validation] ปฏิเสธการเอา 0 วินาทีมาทับเวลาเดิม! (ดึงเวลาเดิม ${maxTimeWeHave}s มาใช้แทน)`);
        dto.currentTime = maxTimeWeHave; // เปลี่ยนค่า 0 เป็นเวลาจริงที่เรามีแทน
      }
    }
  ...
  handleProgressSync() {
    ...
    this.logger.log(`✅ [Cron] ซิงค์และล้าง Cache สำเร็จทั้งหมด ${this.redisCache.size} รายการ`);
  }
